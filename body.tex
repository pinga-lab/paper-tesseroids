...
The distance-size ratio $D$ controls
...
Table~\ref{tab:shell-data} contains the values of the gravitational potential,
attraction, and gravity gradient tensor for such a shell computed at a height
$h = 1\ km$ above the shell.
...
Fig.~\ref{fig:sample}b shows that the largest errors are concentrated directly
...
would not be representative of the actual error.
...
We performed the computations for the tesseroid model on an $11 \times 11$ point regular grid.
...
The grid was located at a constant height $h = 1\ km$ above the spherical shell.

We can take the error in the computation as the maximum absolute difference between the
value calculated for the spherical shell and the corresponding value for the
tesseroid model.

The above computations were repeated for tesseroids of sizes $S = 1^\circ$, $2^\circ$,
$5^\circ$, and $30^\circ$ to verify if the result depends on the discretization.
...
on the Internet\footnote{leouieda.com/tesseroids}

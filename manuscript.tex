...
\todo[inline]{This is an example of how to leave a comment in a Latex
document.}
...
The larger the number of point masses, the better the accuracy of GLQ integration.
...
\todo[inline]{LEO: Valeria, I've made some modifications to your correction.}
...
\todo[inline]{VAL:
Leo, I have modified the above sentence because I have not understood the original meaning.
}
\todo[inline]{LEO: There was a mistake in that sentence.
I've rewritten it. Is it better now?}
...
\todo[inline]{Valeria: Leo,  what does the above sentence mean? Does it mean that these mentioned authors have already used a former version of this software?}
\todo[inline]{Leo: exactly. These are the papers I found that mentioned using Tesseroids.}
...
\todo[inline]{LEO: Valeria, this correction you made is wrong.
It is not ``in regions where the tesseroids are closer''.
It is actually ``in parts of the tesseroid that are closer''.
This is all happening inside a single tesseroid.
...
\todo[inline]{Valeria: Leo, I did not understand "as well as model and computation grid generation"}
\todo[inline]{Leo: I mean that there are programs to generate the model files and the grid files. This can be removed without any problem.}
...
\begin{equation}
    ...
    \label{eq:condition}
\end{equation}
\todo[inline]{Leo: the condition was wrong. It had a $<$ when it should be a $>$. I fixed it.}
...
\todo[inline]{  Regarding the Item 5  - I think that it is confusing.

In the item 5 is written " If none of the dimensions fail the above condition, ...."

This  means that all the dimensions hold the inequality (21). Ok ?

So, Item 5 could be written directly,

5. If all the dimensions hold the inequality (21), hence the tesseroid is not divided and the gravitational effect of the tesseroid is computed by using the
        Gauss-Legendre Quadrature.....}
\todo[inline]{Leo: I agree completely. I have made the modifications below.}
...
\todo[inline]{   Regarding the Item 6b  - I am not sure, but I think that it might be wrong or written in an awkward style
...
\todo[inline]{Leo: Yes, that sentence was a bit confusing.
The idea is checking if the newly generated tesseroids
fit in the stack.
I just wanted to explain how to check for that
(the number of new elements ...).
I'll rephrase it.}
\todo[inline]{Leo: By the way, the proper way of writing " in LaTeX is ``like this''.}
...
\todo[inline]{ VAL: The code was written in C., was not it? I think that there is a  version in Python. However, the readers do not known. }

\todo[inline]{Leo: The code that we are discussing in this paper was written in C.
The figures were generated in IPython notebooks in Python.
The error analysis was done in IPython notebooks as well.
However, it does NOT use the Python version of the tesseroid code.
It runs the C programs and uses Python to calculate the differences
The modeling that is written in Python is not included or discussed here.
Readers do not need to know that it exists.}

\todo[inline]{VAL: The above sentence is confusing:
1) "These files combine the analysis and figure.... ",
What are the files? Probably, they are the IPython notebooks. Ok?
Why are you saying "the analysis"
In the last paragraph, you said "running the analysis".
What is the analysis? Is the test to evaluate the accuracy?
I think that you want to say
"The Ipython nootebook files combine source codes in python, code executions, text, equations, dnd the figures generated by the code into a single document"
You need keeping in mind that you know about Ipython nootebook. However, the readers cannot know.}

\todo[inline]{LEO: Yes, that part is a bit confusing now that I read it again.
By analysis, I mean the error analysis.
That is done in \href{http://nbviewer.ipython.org/github/pinga-lab/paper-tesseroids/blob/master/notebooks/tesseroid_vs_spherical_shell.ipynb}{this IPython notebook}.
This file will be  available  with the article.
There is a link to this online version in \url{https://github.com/pinga-lab/paper-tesseroids}
which is mentioned in the text.
There are two other notebooks that make the figures for the methods and the final results.
There are links to those two in the above link  as well.
I'll rewrite the paragraph to make it more clear.}
...
\url{}\todo[inline]{Replace with the Zenodo/figshare DOI for the repository.}}.
...
\todo[inline]{ VAL: The above paragraph is great !}
...
\todo[inline]{ VAL: A grid of 10 X  10  point covers a are of ??? X ???? km}
\todo[inline]{LEO: The grid location varied in each experiment of the numerical analysis. Table 1 shows the locations of the grids.
This is discussed below.
I'll add this information to the text.}
...
\todo[inline]{ VAL: I rewrite the above sentence by:
"We compute the differences between the effects of the shell and the tesseroid model on the grid; however, we consider  the largest error only.  }
\todo[inline]{LEO: Rewritten. Is it better now?}
...
\todo[inline]{ VAL:I rewrite the above sentence as:
"We decided for the nonconservative procedure of placing the grid on top of a particular tesseroid to increase the chances
of capturing the true largest integration error"}
\todo[inline]{LEO: Changed. I don't think this is really
``nonconservative'' so I left it out.}
...
tesseroid model.
 ...
 \todo[inline]{ Val: Regarding the visual aspect of Figure 4. I think that  the dashed lines are yielding a visual pollution. However, they are helping us to identify the values in the axes. Regarding the visual aspect of Figure 4.
A possibility would be keep the dashed lines, but with  other color (e.g., light gray  dashed lines) }
...
\todo[inline]{ VAL: I think that we discussed that similar results that are achieved in gzz (Figure 5) also are I achieved in gxx, gyy.... I think that this information could be written.}
...
\todo[inline]{ I am here. I did not read the captions of the Figures 4 and 5 and the caption of the Table 1}

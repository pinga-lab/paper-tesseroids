...
The algorithm is described for a single tesseroid and computation point.

The recursive division algorithm implemented with a stack is as follows.

Start by initializing the stack with the tesseroid.

Thus, so far, the stack has only one element.
...
\begin{enumerate}
    ...
    \item If any of the dimensions fail the condition,
        divide the tesseroid in half along each dimension that failed the
        condition.
        Then, place the smaller tesseroids into the stack.
